...
def import_game(request):
    ...
    for player in to_import.get('players', []):
        elts = ((temp_or_perm + "_" + elt, 0) for temp_or_perm in ("temporary", "permanent") for elt in ("sun", "moon", "fire", "air", "water", "earth", "plant", "animal"))
        basic_attrs = {attr: player.get(attr, default) for (attr, default) in (
            ('name', ''),
            ('ready', False),
            ('paid_this_turn', False),
            ('gained_this_turn', False),
            ('color', next(iter(available_colours))),
            ('aspect', ''),
            ('energy', 0),
            ('spirit_specific_resource', 0),
            ('spirit_specific_per_turn_flags', 0),
            *elts,
            )}
        if basic_attrs['color'] in available_colours: available_colours.remove(basic_attrs['color'])
