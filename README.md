...
You do not need one if you are just running the site locally for development.
...
1. Set the Discord channel for a test game to any non-empty value. Suggested to use the Django admin interface for this, since you presumably don't have a running bot listening to a Discord channel at this point.
...
When you're ready to run an instance of the bot and have it post updates to Discord:
...
1. Under Settings → Bot, create a bot for this application, and also do the following:
    ...
    * Create a token for the bot. This token must be provided to `bot.py` as environment variable `DISCORD_KEY`. Recall that `.env` can be used to provide the environment variable.
